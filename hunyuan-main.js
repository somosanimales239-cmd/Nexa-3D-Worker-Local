...
const fs = require('fs');
const { HunyuanConfigStore } = require('./src/backend/hunyuan-config-store');
const { probeSystem } = require('./src/backend/system-probe');
const { HunyuanEngineManager } = require('./src/backend/hunyuan-engine-manager');
const { HunyuanNexaWorker } = require('./src/backend/hunyuan-nexa-worker');
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    show: false,
    webPreferences: {
      preload: path.join(__dirname, 'hunyuan-preload.js'),
...
function registerIpc() {
  ...
  ipcMain.handle('file:pick-image', async () => {
    const result = await dialog.showOpenDialog(mainWindow, {
      title: 'Choose an image for local 3D test', properties: ['openFile'], filters: [{ name: 'Images', extensions: ['png', 'jpg', 'jpeg', 'webp'] }]
    });
    return result.canceled ? null : result.filePaths[0];
  });
  ipcMain.handle('file:pick-apply-zip', async () => {
    const result = await dialog.showOpenDialog(mainWindow, { title: 'Choose a Nexa Apply Package ZIP', properties: ['openFile'], filters: [{ name: 'ZIP files', extensions: ['zip'] }] });
    return result.canceled ? null : result.filePaths[0];
  });
  ipcMain.handle('file:pick-result', async () => {
    const result = await dialog.showOpenDialog(mainWindow, { title: 'Choose the completed result file', properties: ['openFile'], filters: [{ name: '3D Results', extensions: ['glb', 'gltf', 'zip'] }] });
    return result.canceled ? null : result.filePaths[0];
  });
  ...
  ipcMain.handle('path:reveal', async (_event, target) => { if (!target) return { ok: false }; shell.showItemInFolder(String(target)); return { ok: true }; });
  ipcMain.handle('path:open', async (_event, target) => { if (!target) return { ok: false }; await shell.openPath(String(target)); return { ok: true }; });
...
app.whenReady().then(async () => {
  store = new HunyuanConfigStore(app.getPath('userData'), safeStorage);
  engines = new HunyuanEngineManager(store, (line) => send('engine:log', line));
  applyPackages = new ApplyPackageStore(app.getPath('userData'));
  worker = new HunyuanNexaWorker(store, engines, {
  ...
  registerIpc();
  createWindow();
  if (!process.argv.includes('--nexa-ui-smoke') && store.get().auto_start) setTimeout(() => worker.start().catch(() => {}), 1200);
});
