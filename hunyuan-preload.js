...
function ensureHunyuanUi() {
  ...
  if (provider && !provider.querySelector('option[value="hunyuan3d_multiview_local"]')) {
    ...
    option.textContent = 'Hunyuan3D 2mv — Persistent 8082';
  ...
  if (legacyCard && !document.getElementById('hunyuanLocalStatus')) {
    ...
    card.innerHTML = `
      <div class="engine-title"><div class="engine-logo alt">MV</div><div><span class="pill good">RECOMMENDED</span><h3>Hunyuan3D 2mv — Persistent 8082</h3>
  ...
  if (version) version.textContent = 'v1.9.0 · Persistent Hunyuan 8082 · No per-job model reload';
}
