...
function renderApplyPackages(list){
  ...
  const cards=[];
  for(const item of state.applyPackages){
    const remote=Boolean(item.remote_dispatch_id);
    const status=escapeHtml(String(item.status||'imported').toUpperCase());
    const kicker=remote?'AUTO WEB ↔ WORKER BRIDGE':'APPLY PACKAGE';
    const version=escapeHtml(item.version_name||'Current profile');
    const dispatch=remote?' · Dispatch '+escapeHtml(item.remote_dispatch_id):'';
    const resultHint=item.result_name
      ? escapeHtml(formatBytes(item.result_size))
      : (remote?'Drop the finished GLB/GLTF/ZIP into the watched folder for automatic upload.':'Attach the completed GLB/GLTF/ZIP after processing');
    const bridgeBlock=remote
      ? '<div class="apply-brief"><b>Automatic bridge status</b><pre>'+
        escapeHtml(item.bridge_state||'awaiting_result')+'\nWatched folder: '+escapeHtml(item.result_drop_folder||'')+'\n'+
        (item.uploaded_back_to_nexa?'Uploaded back to Nexa automatically.':'The worker will upload the first finished GLB / GLTF / ZIP it finds in the watched folder.')+
        '</pre></div>'
      : '';
    const briefBlock=item.brief
      ? '<div class="apply-brief"><b>Brief preview</b><pre>'+escapeHtml(String(item.brief).slice(0,900))+'</pre></div>'
      : '';
    const watchedButton=item.result_drop_folder
      ? '<button class="btn primary" data-action="open-path" data-path="'+escapeHtml(item.result_drop_folder)+'">Open Watched Result Folder</button>'
      : '';
    const resultButton=item.result_path
      ? '<button class="btn primary" data-action="open-path" data-path="'+escapeHtml(item.result_path)+'">Reveal Result</button>'
      : '';
    cards.push(
      '<article class="card apply-card">'+
        '<div class="card-head"><div><span class="kicker">'+kicker+'</span><h3>'+escapeHtml(item.output_name||item.asset_name||'Apply Package')+'</h3><p class="muted">'+escapeHtml(item.asset_name||'')+' · Version: '+version+dispatch+'</p></div><span class="state-badge '+(item.status==='completed'?'running':'')+'">'+status+'</span></div>'+
        '<div class="metric-grid apply-metrics">'+
          ...
          '<div class="metric"><span>Result file</span><strong>'+escapeHtml(item.result_name||'Not attached')+'</strong><small>'+resultHint+'</small></div>'+
        '</div>'+bridgeBlock+briefBlock+
        '<div class="button-row">'+
          '<button class="btn secondary" data-action="set-status" data-id="'+escapeHtml(item.id)+'" data-status="processing">Mark Processing</button>'+
          '<button class="btn secondary" data-action="set-status" data-id="'+escapeHtml(item.id)+'" data-status="failed">Mark Failed</button>'+
          '<button class="btn secondary" data-action="attach-result" data-id="'+escapeHtml(item.id)+'">Attach Result File</button>'+watchedButton+
          ...
          '<button class="btn ghost" data-action="open-path" data-path="'+escapeHtml(item.zip_path||'')+'">Reveal ZIP</button>'+resultButton+
          '<button class="btn danger" data-action="delete-apply" data-id="'+escapeHtml(item.id)+'">Delete</button>'+
        '</div>'+
      '</article>'
    );
  }
